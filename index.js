...
bot.onText(/\/start/, (msg) => {
    ...
    let Buttons = []
    if (msg.chat.id === config.adminId) Buttons.push([{
        "text": "➕ Ajouter Source",
    ...
    bot.sendMessage(chatId, "<b>🎉 Bienvenue !\n\n👉 Choisissez une option ci dessous</b>", {
...
bot.on("callback_query", async (cb) => {
    ...
    if (Data.action === "addLink") {
        let rssQ = await bot.sendMessage(cb.message.chat.id, "<b>👉 Entrez le lien RSS de la source souhaité</b>", { parse_mode: "HTML" })
        let rssA = await getMessage(bot, cb.message.chat.id, cb.message.chat.id)
        if (rssA === undefined) return
        if (await database.addLinkRSS(rssA.text) != undefined) return bot.sendMessage(cb.message.chat.id, "<b>❌ Impossible d'ajouter le lien, regarder la console pour plus d'information</b>", { parse_mode: "HTML" })
        else bot.sendMessage(cb.message.chat.id, "<b>✅ Lien ajouté avec succès !</b>", { parse_mode: "HTML" })

        await getLink(rssA.text).then(async sourceJson => {
            ...
            if (indexAdded > 0) console.log(`[✔] Nombre d'URLS récemment ajoutées avec succès: ${indexAdded}`.green)
        })

    } else if (Data.action === "subscribe") {
        if ((await database.getUser(cb.message.chat.id)).length != 0) return database.deleteUser(cb.message.chat.id) && bot.sendMessage(cb.message.chat.id, "<b>❌ Vous n'êtes plus sur notre liste de notification!</b>", { parse_mode: "HTML" })
        await database.addUser(cb.message.chat.id)
        bot.sendMessage(cb.message.chat.id, "<b>✅ Vous êtes désormais abonné aux nouvelles notifications!</b>", { parse_mode: "HTML" })
    }
})
